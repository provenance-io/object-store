use crate::consts::{
    MAILBOX_ERROR_RESPONSE, MAILBOX_FRAGMENT_REQUEST, MAILBOX_FRAGMENT_RESPONSE, MAILBOX_KEY,
    NOT_STORAGE_BACKED, SOURCE_KEY, SOURCE_REPLICATION,
};
...
use crate::public_key::{PublicKey, PublicKeyState};
...
impl Datastore for PgPool {
    ...
    async fn get_public_key_object_uuid(&self, hash: &str, public_key: &str) -> Result<uuid::Uuid> {
        let query_str =
            "SELECT object_uuid FROM object_public_key WHERE hash = $1 AND public_key = $2";
    ...
    ON CONFLICT DO NOTHING
        ...
        let object = get_object_by_unique_hash(self, unique_hash.as_str()).await?;
        ...
        match record {
            ...
            Err(sqlx::Error::Database(e)) => {
                if e.code() == Some(std::borrow::Cow::Borrowed("23505")) {
                    self.update_public_key(public_key).await
                } else {
    ...
    async fn get_all_public_keys(&self) -> Result<Vec<PublicKey>> {
        ...
        let mut result_set = sqlx::query(query_str).fetch(self);
    ...
    async fn health_check(&self) -> Result<()> {
        sqlx::query("SELECT 1").fetch_one(self).await?;

        Ok(())
    }

    // METHODS from test below
    // TODO add tests and traces
    ...
    async fn get_public_keys_by_object(&self, object_uuid: &uuid::Uuid) -> Vec<ObjectPublicKey> {
        ...
        let mut result = Vec::new();
        let mut query_result = sqlx::query(query_str).bind(object_uuid).fetch(self);
...
/// 2. Connects to [DbConfig::db_connection_string]
/// 3. Migrates database
pub async fn init_postgres(config: &DbConfig) -> Result<Arc<PgPool>> {
