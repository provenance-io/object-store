...
impl Dime {
    ...
    pub fn unique_audience_without_owner_base64(&self) -> Result<Vec<String>> {
        let mut audience = self.unique_audience_base64()?;
        let owner = self.owner_public_key_base64()?;

        audience.retain(|item| item != owner.as_str());
...
impl TryFrom<Bytes> for Dime {
    ...
    fn try_from(buffer: Bytes) -> Result<Self> {
        ...
        let size_err = Err(DimeError::BufferSizeError(
            "Not enough bytes in buffer to parse Dime".to_owned(),
        ));
        ...
        if magic_bytes != MAGIC_BYTES {
            return Err(DimeError::InvalidMagicBytesError(format!(
                "Invalid magic bytes of {}",
                magic_bytes
            )));
        }
        ...
        if buffer.remaining() < uuid_len as usize {
            return size_err;
        }
        ...
        if buffer.remaining() < metadata_len as usize {
            return size_err;
        }
        ...
        if buffer.remaining() < uri_len as usize {
            return size_err;
        }
        ...
        if buffer.remaining() < signature_len as usize {
            return size_err;
        }
        ...
        if buffer.remaining() < proto_len as usize {
            return size_err;
        }
...
mod tests {
    ...
    fn magic_bytes() {
        ...
        assert_eq!(
            actual,
            Err(DimeError::InvalidMagicBytesError(format!(
                "Invalid magic bytes of {}",
                257
            )))
        );
    ...
    fn complete_parse() {
        ...
        let proto = pb::Dime {
            ...
            payload: vec![pb::Payload {
                id: 1,
                cipher_text: vec![1, 2, 3, 4, 5, 6, 7, 8],
            }],
            audit_fields: None,
        };
    ...
    fn encode_to_decode_matches() {
        ...
        let actual = format_dime_bytes(&mut buffer, signature);
        assert!(actual.is_ok());
